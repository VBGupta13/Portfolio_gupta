...
if selected == "Home":
    ...
    with st.container():
        ...
        with matter:
            ...
            st.write("<div style='font-size: 18px;'>Hi, I'm VEERA BHADRA GUPTA CHARUGALLA. I'm currently pursuing B.Tech 4th year in the department of Computer Science and Engineering at Sri Vasavi Engineering College, Pedatadepalli affiliated to JNTUK</div>", unsafe_allow_html=True)
            st.text("")
            st.write("<div style='font-size: 18px;'>▶ In addition I'd like to play table tennis 🏓 , badminton, listening music 🎧 and...  enjoy eating food 🍽️ in my free time!</div>", unsafe_allow_html=True)
            st.text("")
            st.markdown("<div><h4>Social Media</h4></div>",unsafe_allow_html=True)
            st.markdown("""
                -  [linkedIN](www.linkedin.com/in/veera-bhadra-gupta-charugalla-b0b994245)
                -  [Github](https://github.com/VBGupta13/)
                """)
    ...
    with right:
        ...
        with image_column:
            st.image("Edunet.png")
        with text_column:
            st.subheader("Web Developer , Edunet foundation Intern")
            st.write("*01-2023 to 02-2023*")
            st.markdown("""
    ...
    st.markdown("<div style='text-align:center;margin-top:10px'><h4>MAIL-ID : </h4><h6>guptach1105@gmail.com</h6></div>",unsafe_allow_html=True)
    st.markdown("<div style='text-align:center;margin-top:10px'><h4>MOBILE MUMBER : </h4><h6>+91-93929-68736</h6></div>",unsafe_allow_html=True)
    if state:
    ...
    with st.container():
        ...
        with col3:
            st.image("Sri.jpeg", use_column_width=True)
    ...
    with open("Veera_Bhadra_Gupta_Charugalla.pdf", "rb") as file:
        btn = st.download_button(
            label="Download Resume",
            data=file,
            file_name="Veera_Bhadra_Gupta_Charugalla.pdf",
